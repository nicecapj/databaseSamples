//reference : https://docs.microsoft.com/en-us/sql/odbc/reference/sample-odbc-program?view=sql-server-ver15
#include <windows.h>
#include <iostream>
#include <conio.h>
#include <sql.h>
...
{	
	SQLHENV     hEnv = NULL;	//sql 환경
	SQLHDBC     hDbc = NULL;	//sql 연결(void*)
	SQLHSTMT    hStmt = NULL;	//sql구문
	WCHAR*      pwszConnStr;
	WCHAR       wszInput[SQL_QUERY_SIZE];	//최대 1000자 sql쿼리
	...
	//할당한 핸들을 사용해서 sql환경속성 설정한다(odbc ver3.x이상으로)
	TRYODBC(hEnv, SQL_HANDLE_ENV, SQLSetEnvAttr(hEnv, SQL_ATTR_ODBC_VERSION,(SQLPOINTER)SQL_OV_ODBC3, 0));
	//signed short retcode = SQLSetEnvAttr(hEnv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3, 0);
	...
	//else
	//{
	//	fwprintf(stderr, L"Error in SQLSetEnvAttr);
	//	goto Exit;
	//}
	...
	//ODBC 드라이버에 연결한다. 
	//지정된 이름(pwszConnStr)이 없으면, 데이터 원본(dsn)선택하는 창 출력된다.
	//mysql처럼 주소, id, port등을 입력할 필요가 없다. 시스템에 설정된 dsn에 이 정보가 있다.
	TRYODBC(hDbc,
		SQL_HANDLE_DBC,
	...
	wcscpy_s(wszInput, 256, L"select * from account;");
	...
	switch (retCode)
	{
		...
		case SQL_SUCCESS:
		{
			...
			TRYODBC(hStmt,
				SQL_HANDLE_STMT,
				SQLNumResultCols(hStmt, &numResult));

			if (numResult > 0)
			{
				DisplayResults(hStmt, numResult);
			}
			else
			{
				SQLLEN cRowCount;
